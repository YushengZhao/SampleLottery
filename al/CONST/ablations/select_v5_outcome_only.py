# al/CONST/ablations/select_v5_outcome_only.py
import json
import os
import argparse
import pandas as pd
...
def main():
    parser = argparse.ArgumentParser(description="Ablation V5: Use Outcome Volatility only for selection.")
    ...
    # Recalculate q_hat using only Outcome Volatility answers
    try:
        ...
        calib_results_path = os.path.join(model_output_dir, "calibration_results_outcome_volatility.jsonl")
    ...
    tqdm.pandas(desc="Rescoring Pool (Outcome Only)")
    ...
    data_source_name = "CONST_V5_OutcomeOnly"
